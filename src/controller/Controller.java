    ...
    private void computeIntegral(ActionEvent actionEvent) {
        if(checkFields()) {
            ...
            for (int i = 0; i < ordinaryLeastSquares.getN(); i++) {
                series.getData().add(new XYChart.Data<>(ordinaryLeastSquares.getXi().get(i).toString(), ordinaryLeastSquares.getYi().get(i)));
            }
            i_graph_1.getData().add(series);
            series = new XYChart.Series<>();
            for (int i = 0; i < ordinaryLeastSquares.getN(); i++) {
                series.getData().add(new XYChart.Data<>(ordinaryLeastSquares.getXi().get(i).toString(), ordinaryLeastSquares.getYiDb().get(i)));
            }
            i_graph_1.getData().add(series);
            ...
            i_resultsInfo_2.setText("Ошибка");
        }
    }
